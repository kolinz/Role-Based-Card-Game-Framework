...
## ゲーム化の例
職種を役割（ロール）として見立て、ロールに必要なスキル集めと、グループワークの組み合わせができれば、だいたい対応できます。

たとえば、下記は実現できます。
- 探究学習支援や進路指導、キャリア教育の場合
  - ICT関連職 （初期のサンプルデータ）
  - 高校教員
- 専門分野学習の場合
  - ハイブリッドクラウド
...
- ✅ ３種類のカード（職種カード、スキルカード、ミッションカード）を使ったオンラインカードゲームプラットフォーム。
  ...
  - ミッションカード：業務疑似体験カード。スキルカードと一緒に表示される。カードに書かれた疑似体験業務を、他のプレイヤーと協力して遂行する。ミッションカードは4種類ある。
   <img width="1814" height="941" alt="image" src="https://github.com/user-attachments/assets/045a11c5-4969-4271-b71e-5d7d02e7c358" />
- ✅ サンプルデータとして、以下のICT関連職種を学べるデータセット <-- 管理画面を使うことで、ICT関連職種以外も実現できます。
  - 職種カード
    - ソフトウェアエンジニア
    - クラウドアーキテクト
    - DevOpsエンジニア
   ...
   - ミッションカード
     - 職種カードに関係する業務疑似体験
     <img width="1119" height="900" alt="image" src="https://github.com/user-attachments/assets/7fefe31c-3a60-41e1-8855-dc0a17d56f2d" />
   - カテゴリ
     - カッツモデルにもとづく、ミッションカードのカテゴリ3種
       ...
       - コンセプチュアルスキル
     - 予期せぬ事態への対応
     <img width="1147" height="910" alt="image" src="https://github.com/user-attachments/assets/91dabd22-9dfd-4040-9db9-0c1a3f602ea1" />
...
簡易版 : [wiki](https://github.com/kolinz/Role-Based-Card-Game-Framework/wiki)の「ゲームのはじめかた」 をご覧ください

詳細版は下記です。

1. 2～4名のグループをつくる。
2. ゲームの部屋（セッション）を発行し、招待URLをグループメンバーに送信。
3. グループメンバーがゲーム画面に入る。ゲーム画面に入る際に職種カードを1枚選ぶ
4. 招待URLに参加した順に、ゲームが進む
5. 「サイコロを振る」をタップ（クリック）
6. でたスキルカードおよびミッションカードから、1枚を選ぶ。スキルカードが職種に関係する場合は、プレイヤーのポイントが1ポイント増える。ミッションカードはグループワーク。
7. 次のターン = 次の人へ
8. 5～7を繰り返す。
9. 職種カードごとに決められたポイントを発生すればゲーム終了。全員がゲーム終了となるまでまつ。
10. 振り返りワークシートを作成

<a id="anchor4"></a>
## 管理画面について

ゲームのURの後に /admin.html をつけることで管理画面にログインすることができます。導入時に作成する「.env」ファイルで制御、管理画面にログインするためのユーザー名とパスワードを制御しています。

<img width="480" height="640" alt="image" src="https://github.com/kolinz/Role-Based-Card-Game-Framework/blob/main/docs/images/game-admin-1.png" />
...
### プレイヤー
  - Webブラウザのみ（インストール不要） 
### 📋 前提環境
https://nodejs.org/ja/download を参考に、Node.js 24 をインストールします。
ここでは、下記のパスにNnode.jsがインストールされたとします。
```text
...
ExecStart=/home/ubuntu/.nvm/versions/node/v24.16.0/bin/node /home/ubuntu/Role-Based-Card-Game-Framework/server.js
Restart=on-failure
RestartSec=10
